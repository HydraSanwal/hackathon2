...
(function() {
  ...
  function createChatWidget() {
    ...
    script.src = 'data:text/javascript,' + encodeURIComponent(`
      ...
      // Initialize the chat icon when the page loads
  ...
  async function sendMessage(message) {
    try {
      ...
      // Add bot response to chat (you'll need to adapt this to your chat UI)
      const chatIcon = window.chatIconInstance;
      if (chatIcon) {
        chatIcon.addMessageToChat(data.response, 'bot');
      }
    } catch (error) {
      console.error('Error sending message:', error);
    }
